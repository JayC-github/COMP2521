...
// create a new node include the string
...
void releaseTB(TB tb) {
    ...
    // free all the node in the textbuffer
    Node *curr = tb->first;
    while (curr != NULL) {
        Node *tmp = curr;
        curr = curr->next;
...
char *dumpTB(TB tb, bool showLineNumbers) {
    ...
    while(curr != NULL) {
        ...
        if (showLineNumbers == true) {
            ...
            add = realloc(add, strlen(curr->text) + size + 1);                        
...
void mergeTB(TB tb1, int pos, TB tb2) {
    if (tb1 == NULL || pos < tb1->first->line 
    || pos > tb1->last->line + 1) {
        fprintf(stderr,"Out of range\n");
        abort();
    }
...
void pasteTB(TB tb1, int pos, TB tb2) {
   ...
   //Insert tb3 before the start of tb1
    if (pos == 1) {
        Node *tmp = tb1->first;
        tb1->first = tb3->first;
        Node *curr = tb1->first;
        // while loop till the end of new tb1 (tb3);
        while (curr->next != NULL) {
            curr =curr->next; 
        }
        // linked tb1 in the end of tb3
        curr->next = tmp;
        tb1->numLine = tb1->numLine + tb3->numLine;
        free(tb3);
        return;  
    }
    
    // Insert tb3 after the end of tb1
    if (pos == (tb1->numLine + 1)) {
        Node *curr = tb1->first;
        Node *head = tb3->first;
        while (curr->next != NULL) {
    ...
    // link the tb3 at the 'pos' line
    curr->next = head;
    
    // loop tb3 till the end of the linked list
    while(head->next != NULL) {
        head = head->next;
    }
...
void deleteTB(TB tb, int from, int to) {
    ...
    Node *after = NULL;
    Node *curr = tb->first;
    // before point to the node one position before the 'from'
    // after point to the node one position after the 'to'
    while (curr != NULL) {
        if (curr->line == from - 1) {
            before = curr;
